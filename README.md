...
- **透视修正插值** — UV / 法线使用 clip.w 做深度矫正，消除大三角形纹理拉伸
- **阴影映射** — 两趟渲染：光源视角记录深度，相机视角查表判定阴影
...
| `simplegl.cpp/hpp` | 渲染管线：光栅化、深度测试、透视投影、坐标变换、阴影映射 |
...
6. **Perspective-correct interpolation** — UV 和法线使用 clip.w 做深度矫正插值，消除大三角形纹理拉伸
7. **Shadow mapping** — 两趟渲染实现阴影：光源视角记录深度到 lightzbuffer，相机视角查表判定阴影，提取 TriangleSetup 消重复
